...
When a material is illuminated by monochromatic light of frequency $\omega$, the incident light is either absorbed, scattered, reflected, or transmitted. While the spectrum of the transmitted and reflected light contains only light at frequency $\omega$, the spectrum of the scattered light includes both radiation at $\omega$ and additional pairs of frequencies $\omega \pm \omega_{i}$. The frequencies $\omega_{i}$ are typically associated with transitions between rotational, vibrational, and electronic states of the constituent molecules of the material \cite{Long2002}. The scattered light typically has a randomized phase relative to the incident light.
...
Although to this point we have focused on Raman scattering from crystals, single molecules and liquids can also cause Raman scattering. Accordingly, the Raman spectrum of an electrolyte must be considered when measuring the Raman spectrum of an electrolyte-gated material. I have characterized the Raman spectra of several electrolytes to determine which is most appropriate for measurements. Figure \ref{fig:RamanSpectElec-1} shows these Raman spectra.
...
Because with one exception the selected electrolytes would add substantial background to a Raman spectrum or would have features obscuring those of \ruclnospace , I performed the following measurements with DEME-BF4.
...
Samples were attached to an 8 contact DIP chip carrier using poly(methyl methacrylate). Gold wirebonds connected the sample to the chip carrier, which was mounted on a breadboard. A drop of electrolyte that covered only the coplanar gate and the \rucl flake was manually applied using a pipette tip. Potential bias was applied using a Keithley 2400 source measurement unit in voltage source mode controlled by GPIB. Applied gate voltage and resulting gate current were recorded during the measurement.
...
Raman spectra of \rucl measured under a biased electrolyte divide into two distinct classes. The first, which I refer to as \pplus , has fewer and broader peaks than pristine \ruclnospace. The second, \pminus , is almost identical to pristine \ruclnospace. Figure \ref{fig:RuCl3RamanResults-1} shows these different spectra. Each spectrum can be accessed by changing the gate voltage. \pplus{} typically appears at positive electrolyte gate voltage, and \pminus{} appears at negative gate voltage.
...
\begin{figure}[p]
	...
	\end{minipage}
	\begin{minipage}{0.5\textwidth}
		\captionof{figure}[Raman spectra of pristine \ruclnospace , \pminus{}, and \pplus]{Backgronund-subtracted Raman spectra of (a) pristine \rucl without electrolyte applied, (b) \rucl with electrolyte applied and spectrum \pplus, and (c) \rucl with electrolyte applied in spectrum \pminus. Black lines are data, blue lines are Lorentzians simultaneously fit to the data, and red lines are the sum of the Lorentzians. Features near 400 and 520 cm\textsuperscript{-1} are from the substrate. The spectra for pristine \rucl and \pminus{} are nearly identical.}
...
\begin{center}
	...
	\end{tabular}
	\captionof{table}[Quantitative comparison of Raman spectra \pminus{} and \pplus]{Quantitative comparison of Raman spectra for a particular device for pristine \ruclnospace , \pminus , and \pplus . Red X's indicate the peak is not present or too small to fit in \pplus . Peak 8 does not correspond to a symmetry of \rucl but is required for a convergent fit.}
	\label{tbl:RuCl3RamanResults-1}
\end{center}

Table \ref{tbl:RuCl3RamanResults-1} presents a quantitative comparison of the three different spectra for a particular \rucl device. Peaks 1 through 7 have only small, random changes between pristine \rucl and \pminus approximately equal to the $\pm${} 1 cm\textsuperscript{-1} uncertainty of the fit, suggesting these peaks are identical. Peak 8, which is not related to a symmetry of \rucl but must be included for the fit to converge, broadens substantially in \pminus . Peak 8 may be a surface mode which is disordered by the application of the electrolyte.

\pplus{} is substantially different from pristine \rucl and \pminus . Several peaks are absent in \pplus , and the remaining peaks are broader.

\subsubsection{Transition between different Raman spectra is hysteretic}

Electrolyte-biased \rucl switches between spectra \pminus{} and \pplus{} as a function of gate voltage. \pplus{} emerges from the pristine \rucl spectrum as the electrolyte gate voltage is increased from zero to slightly positive. The spectrum remains in \pplus{} until the gate voltage is large and negative (typically V\textsubscript{crit} $\approx$ -1.5 V) when the spectrum abruptly switches back to \pminus . This effect can be seen multiple times as the voltage is swept on a given sample. Plots of relative intensity and Raman shift for peak 2 presented in Figure \ref{fig:RuCl3RamanResults-2} show the hysteresis in this transition.
...
The visual appearance of the flake changes abruptly when the spectrum moves from pristine to \pplus{} to \pminus . Electrolyte-biased \rucl with Raman spectrum \pplus{} has more optical absorption than electrolyte-biased \rucl with Raman spectrum \pminus . Further, as the voltage is swept across V\textsubscript{crit}, an interface between the two regions sweeps across the flake, as shown in Figure \ref{fig:RuCl3RamanResults-3}. While the optical absorption changes with electrolyte bias (i.e., \rucl having spectrum \pminus{} is ``lighter''), the mottled appearance and colors of the flake are irreversible.


\begin{centering}
\includegraphics[width=\textwidth]{./img/RamanTransitionInterface.png}
  \captionsetup{width=0.75\textwidth}
  \captionof{figure}[Change in visual appearance correlated with change in Raman spectra]{Change in visual appearance as electrolyte bias drives the Raman spectra between \pminus{} and \pplus. Images were taken at a constant gate voltage of 0.02 V after the gate voltage was held at -1.2 V. Note the interface between the light and dark portions of the flake in the center image.}
  \label{fig:RuCl3RamanResults-3}
\end{centering}

\subsection{Discussion}

Electrolyte-biased \rucl has two distinct Raman spectra: one is identical to pristine \ruclnospace , the other has fewer, broader peaks. The spectrum switches hysteretically between these two spectra as a function of gate voltage. The spectra are also associated with a lighter (\pminus) and a darker (\pplus) visual appearance. An interface between \pminus{} and \pplus{} regions appears as electrolyte biasing drives the change in Raman spectrum.

As discussed earlier in this dissertation, hysteresis and an interface are the hallmarks of a first-order phase transition. Further, the range of electrolyte gate voltage associated with hysteresis in conductance roughly matches that found in Raman, despite using a different technique and using a different sweep rate.

These results are consistent with the hypothesis that a phase transition results from adding charge carriers. However, while the Raman measurements support the phase transition hypothesis, they fail to disprove intercalation or rule out the presence of electrochemical reactions. To address these remaining possibilities, we turn to x-ray diffraction.